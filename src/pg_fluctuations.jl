...
function calculate_performance_measures(f, RoCoFs; T, Δt = 0.01)
    ω = f .* 2π
    f_max = findmax(f)[1]
    f_min = findmin(f)[1]

    RoCoF_max = findmax(RoCoFs[Int(10/Δt):end, :])[1]
    RoCoF_min = findmin(RoCoFs[Int(10/Δt):end, :])[1]
